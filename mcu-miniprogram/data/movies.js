 ...
 * MCU 宇宙导航（小程序） - 电影主数据
 * ------------------------------------------------------------
 * 来源：H5 mcu-navigator/data/movies.js（唯一可信源，机械适配）
 * 适配方式：window.MCU_MOVIES = [ ... ]  →  module.exports（去 window 前缀）
 * 数据内容 100% 一致，仅改变加载方式。禁止在此重新整理/修改数据。
 *
 * 数据口径：本文件仅收录 MCU 院线电影（38 部），不含 Disney+ 剧集/特别呈现/短片。
 * 截至 2026-08，已上映院线电影共 38 部
 * （《钢铁侠》2008-05-02 → 《蜘蛛侠：崭新之日》2026-07-31）。
 * 全量 59 个 MCU 内容（电影+剧集+特别呈现+短片）由 data/content.js 统一合成。
 *
 * 字段说明：
 ...
 *   ro       上映顺序序号 1-38
 *   co       故事时间线顺序序号 1-38（content.js 统一重算）
 ...
 *   chars    出场重点角色 id
 *   next     手写的下一部推荐；未写的由 models/recommend.js 自动兜底
 * ============================================================ */
...
module.exports = { MOVIES, UPCOMING };
