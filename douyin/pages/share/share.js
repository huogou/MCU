...
Page({
  ...
  /* ---- 保存到相册（抖音 V2 canvas 适配：toDataURL → 写文件 → 存相册） ---- */
  savePoster: function () {
    var that = this;
    if (!this._canvas) {
      tt.showToast({ title: '海报生成中，请稍后', icon: 'none' });
      return;
    }
    try {
      ...
      var base = (tt.env && tt.env.USER_DATA_PATH) || '';
      var filePath = base + '/mcu_poster_' + Date.now() + '.png';
            ...
            fail: function (err) {
              ...
              if (/auth|deny|permission/i.test(msg)) {
                tt.showModal({
                  ...
                  confirmText: '去设置',
                  success: function (r) { if (r.confirm) tt.openSetting(); }
                });
              } else {
                tt.showToast({ title: '保存失败，请重试', icon: 'none' });
              }
            }
          });
        },
