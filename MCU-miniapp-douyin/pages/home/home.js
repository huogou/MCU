...
Page({
  ...
  /* Hero 迷你旅程条 → 路线页（Tab） */
  goJourney() {
    tt.switchTab({ url: '/pages/routes/routes' });
  },

  /* 功能入口 2×2 → 对应页面（路由对齐实际页面；Tab 页用 switchTab） */
  onEntryTap(e) {
    const key = e.currentTarget.dataset.key;
    if (key === 'watch') tt.switchTab({ url: '/pages/routes/routes' });
    else if (key === 'timeline') tt.navigateTo({ url: '/pages/panorama/panorama' });
    else if (key === 'characters') tt.navigateTo({ url: '/pages/characters/characters' });
    else if (key === 'relationships') tt.switchTab({ url: '/pages/explore/explore' });
  },
