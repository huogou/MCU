/* ============================================================
 * MCU 宇宙导航（小程序） - 内容模型合成器（CONTENT）
 * ------------------------------------------------------------
 * 来源：H5 mcu-navigator/data/content.js（唯一可信源，机械适配）
 ...
 *   - 类型/重要度常量为全局唯一来源。
 *
 * 适配说明：H5 以 IIFE + window 挂载；小程序以 require/module.exports
 * 加载，合成算法与数据内容 100% 一致，仅改变挂载方式。
 * ============================================================ */
...
const TYPE = { MOVIE: 'movie', SERIES: 'series', SPECIAL: 'special', SHORT: 'short' };
const TYPE_LABEL = { movie: '电影', series: '剧集', special: '特别呈现', short: '短片' };

/* —— 重要度常量（必看/推荐/可选）——
 * 电影：mainline → core；starter → recommended；其余 → optional。
 * 剧集/特别篇/短片：各自数据文件已标 importance，原样保留。 */
const IMPORTANCE = { CORE: 'core', RECOMMENDED: 'recommended', OPTIONAL: 'optional' };
const IMPORTANCE_LABEL = { core: '必看', recommended: '推荐', optional: '可选观看' };
const IMPORTANCE_RANK = { core: 0, recommended: 1, optional: 2 };

/* 电影派生 type/importance（不动源文件） */
const movies = MOVIES.map(function (m) {
  const imp = m.mainline ? 'core' : (m.starter ? 'recommended' : 'optional');
  const copy = {};
  for (const k in m) if (m.hasOwnProperty(k)) copy[k] = m[k];
  copy.type = 'movie';
  copy.importance = imp;
  return copy;
});

const content = movies.concat(SERIES, SPECIAL, SHORT);
...
content.forEach(function (c) {
  for (const k in DEFAULT_SOURCE) if (!(k in c)) c[k] = DEFAULT_SOURCE[k];
});

/* 故事时间线序（tl / co）：四类内容统一用「故事年份」作主排序键。
 * 主排序键取自 coLabel 中的年份（电影与剧集/特别呈现/短片用同一把尺子），
 * 缺 coLabel 时回退到发行年。同一年内：电影用其策划 co 保证年内次序精确；
 * 其余用发行日期作次级序。 */
function storyYear(c) {
  if (c.coLabel) { const y = parseInt(c.coLabel, 10); if (!isNaN(y)) return y; }
  return c.year || 0;
}
function storySub(c) {
...
content.sort(function (a, b) {
  const ya = storyYear(a), yb = storyYear(b);
